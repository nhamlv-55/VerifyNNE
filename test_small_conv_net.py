...
class ToyConvNet(BaseNet):
    def __init__(self):
        super().__init__()
        self.conv1 = torch.nn.Conv2d(in_channels=1, out_channels=2, kernel_size=3)
...
def run_toy():
    ...
    input = torch.autograd.Variable(torch.Tensor(
        ...
        [0.21, 0.22, 0.23, 0.24],
        [0.31, 0.32, 0.33, 0.34],
        [0.41, 0.42, 0.43, 0.44]]]]), requires_grad=True)
    ...
    ipq = build_saliency_mask_query(
        toy, dummy_input=torch.randn(1, 1, 4, 4), input=input, target=0)
